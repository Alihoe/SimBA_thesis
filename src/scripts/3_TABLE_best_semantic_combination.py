...
def run():
    ...
    with open(dir_of_file + "/output/3_TABLE_best_semantic_combination.txt", 'w') as f:
        print(scores_df.style.format_index(axis=1, formatter="{}".format).hide(
            axis=0).format(precision=3).to_latex(column_format=column_format, position="!htbp",
                                                 label="table:combo_only_semantic",
                                                 caption="Highest Scores for Combinations of Semantic Features only.",
